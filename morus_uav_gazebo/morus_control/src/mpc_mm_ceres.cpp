        ...
        for (int i = 0; i< pred_horizon; i++){
           ...
           lambdas_x.block(0,i,x0_.rows(), x0_.cols())  = -1*x_ss_ + x_states.block(0,i,x0_.rows(), x0_.cols());
            ...
            if(i < control_horizon){
                ...
                deriv_wrt_u.block(0,control_horizon-1,u.rows(), u.cols()) += (2*R_*u) -2*R_*u_ss_  + (4*R_delta_*u)+ (-2*R_delta_*u_past);
            
                deriv_wrt_u.block(0,control_horizon-1,u.rows(), u.cols()) += (deriv_wrt_u.block(0,4,u.rows(), u.cols()) -2*R_delta_*u).eval();
            ...
            for(int j = 0; j<= i; j++){
                ...
                deriv_wrt_u.block(0,j,u.rows(), u.cols()) +=((2*Q_*x_states.block(0,i+1,x0_.rows(), x0_.cols()) - 2*Q_*x_ss_).transpose()*A_pow_B_cache.block(0,(i-j),A_.rows(), B_.cols())).transpose();
            
                    
                }else{
                    if(j >= control_horizon){
                    deriv_wrt_u.block(0,4,u.rows(), u.cols()) += ((2*Q_*x_states.block(0,i+1,x0_.rows(), x0_.cols()) - 2*Q_*x_ss_).transpose()*A_pow_B_cache.block(0,(i-j),A_.rows(), B_.cols())).transpose();
