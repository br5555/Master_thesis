...
RMS_Prop_MPC::RMS_Prop_MPC(Matrix<double, num_state_variables, num_state_variables> A, Matrix<double, num_state_variables, num_manipulated_variables>  B,
...
Matrix<double, 2 * mpc_control_horizon, 1>& RMS_Prop_MPC::Evaluate(Matrix<double, 2 * mpc_control_horizon, 1>&   x) {
	...
	for (int iter = 0; iter < max_iter; iter++) {
		...
		for (int RMS_Prop_iter = 0; RMS_Prop_iter < num_heuristic_variables*mpc_control_horizon; RMS_Prop_iter++) {
			change_x(RMS_Prop_iter, 0) = -1 * ( epsilon * gradients(RMS_Prop_iter, 0) ) / ( sqrt(gradient_past(RMS_Prop_iter, 0)) + delta );
		}
...
RMS_Prop_MPC::~RMS_Prop_MPC()
{
}

Matrix<double, 2 * mpc_control_horizon, 1>& RMS_Prop_MPC::check_bounderies(Matrix<double, 2 * mpc_control_horizon, 1>&   x) {
	dummy_u = u_current;
