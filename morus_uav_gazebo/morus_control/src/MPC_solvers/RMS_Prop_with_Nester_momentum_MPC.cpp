...
RMS_Prop_with_Nester_momentum_MPC::RMS_Prop_with_Nester_momentum_MPC(Matrix<double, num_state_variables, num_state_variables> A, Matrix<double, num_state_variables, num_manipulated_variables>  B,
	...
	Matrix<double, num_manipulated_variables, num_manipulated_variables> scale_MV, Matrix<double, num_state_variables, num_state_variables> scale_OV, double sampingTime) : num_params_(num_params), pred_horizon(pred_horizon), control_horizon(control_horizon), A_(A), B_(B), Bd_(Bd), Q_(Q), Q_final_(Q_final), R_(R), R_delta_(R_delta), disturbance_(disturbance)
{
	...
	du_limit(0, 0) = sampingTime * 2;
	du_limit(0, 1) = 1*(sampingTime/0.01);
	du_limit(1, 0) = -sampingTime * 2;
	du_limit(1, 1) = -1*(sampingTime/0.01);
...
Matrix<double, 2 * mpc_control_horizon, 1>& RMS_Prop_with_Nester_momentum_MPC::Evaluate(Matrix<double, 2 * mpc_control_horizon, 1>&   x) {
	...
	for (int iter = 0; iter < max_iter; iter++) {
		...
		gradient_past = (rho*gradient_past + (1 - rho)*gradients.cwiseProduct(gradients)).eval();
		...
		for (int RMS_Prop_iter = 0; RMS_Prop_iter < num_heuristic_variables*mpc_control_horizon; RMS_Prop_iter++) {
			change_x(RMS_Prop_iter, 0) =( alpha* change_x(RMS_Prop_iter, 0) -1 * (epsilon * gradients(RMS_Prop_iter, 0)) / (sqrt(gradient_past(RMS_Prop_iter, 0)) + delta));
		}
...
RMS_Prop_with_Nester_momentum_MPC::~RMS_Prop_with_Nester_momentum_MPC()
{
}

Matrix<double, 2 * mpc_control_horizon, 1>& RMS_Prop_with_Nester_momentum_MPC::check_bounderies(Matrix<double, 2 * mpc_control_horizon, 1>&   x) {
	dummy_u = u_current;
